var src_adj, size, links, path, circle, text, force, container;


$( document ).ready(function() {
  src_adj = $("#src_adj").text();
  size = $("#size").text();
  render();


  $("#goButton").click(function() {
    window.location.href = '/view-adjectives/' + $("#newAdjSrc").val() + '/' + $("#newSize").val();
  })
...
function render() {
  ...
  $.getJSON( '/get/adjectives/' + src_adj + '/' + size, function( data ) {
  ...
  }).error(function() { $("#error").html("There were no edges stemming from \"" + src_adj + "\"") });
...
function draw() {
  if (links.size == 0) {
    $("#error").html("There were no edges stemming from \"" + src_adj + "\"")
    return
  }
  var nodes = {};
